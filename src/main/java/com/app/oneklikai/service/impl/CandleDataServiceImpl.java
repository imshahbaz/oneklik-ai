...
public class CandleDataServiceImpl implements CandleDataService {
    ...
    public void trainModel(String symbol, int sequenceLength, int epochs, float learningRate) {
        ...
        if (!CsvService.candleSticks.containsKey(stockSymbol)) {
            throw new IllegalArgumentException("No candles loaded for symbol: " + stockSymbol);
        }

        List<YahooCandleStick> candles = CsvService.candleSticks.get(stockSymbol);
        DatasetBuilder.TrainingBatch batch = DatasetBuilder.buildSlidingWindows(candles, sequenceLength);
        ...
        try (NDManager manager = NDManager.newBaseManager()) {
            ...
            // Set Network Block
            model.setBlock(DataTransformerBlock.buildArchitecture(featureDim, embedDim));
    ...
    public PredictionResponse predictNextDay(Model trainedModel, String symbol, int sequenceLength) {
        ...
        List<YahooCandleStick> allCandles = CsvService.candleSticks.get(stockSymbol);
        if (allCandles.size() < sequenceLength) {
            throw new IllegalArgumentException("Insufficient data. Require at least " + sequenceLength + " candles.");
        }

        // 1. Extract the LAST 60 candles to serve as the prompt context
        List<YahooCandleStick> last60 = allCandles.subList(allCandles.size() - sequenceLength, allCandles.size());
        YahooCandleStick latestCandle = last60.getLast();
        double basePrice = last60.getFirst().open(); // Anchor base price for local normalization
        ...
        double currentClose = latestCandle.close();
