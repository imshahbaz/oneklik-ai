...
public class CandleDataController {
    ...
    /** Trains the daily model and returns its out-of-sample scores. Hyperparameters come from {@link Constants}. */
    @PostMapping("/train")
    public ResponseEntity<ApiResponse<TrainingReport>> train(@RequestParam("symbol") @NotBlank String symbol) {
        TrainingReport report = candleDataService.trainModel(
                symbol,
                Constants.DAILY_SEQUENCE_LENGTH,
                Constants.DAILY_EPOCHS,
                Constants.DAILY_LEARNING_RATE);
